...
if(isset($_POST['donate']) ){
	...
	if(!empty($full_name) && !empty($amount) && !empty($card_number) && !empty($card_name) && !empty($email) && !empty($address) && !empty($type) && !empty($date)){
       ...
       //insert donated info into section donator table
		$sql="INSERT INTO `section_donator`( `full name`,`email`,`address`,`city`,`card name`,`card number`,`expire month`,`amount`,`section type`) VALUES ( '$full_name', '$email','$address','$city','$card_name','$card_number','$date','$amount','$type')";
